...
use server::{ServerConfig, Command, KvServer};
...
fn main() {
     ...
     let sc1 = create_server_config(1, vec![1, 2, 3]);
        ...
        router1.propose_raft_command(Command::Del(b"key1".to_vec())).unwrap();
        router2.propose_raft_command(Command::Del(b"key2".to_vec())).unwrap();
        router3.propose_raft_command(Command::Del(b"key3".to_vec())).unwrap();
